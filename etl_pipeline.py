...
open_whether_key = config.get("open_whether_key")
# conf1 = extract_config


# MongoDB setup
# client = MongoClient("mongodb+srv://isbah:5Do3z8X2oTXNeEab@python-db.ua5uu.mongodb.net/")
# mongo_db = client["project_db"]
# collection = mongo_db["weather_db"]
...
def extract_sql():
    engine = create_engine("sqlite:///data/sample_sql.db")  # Replace with actual DB
    return pd.read_sql("SELECT * FROM weather_table", con=engine)
...
def run_etl():
    print("🌦️ Starting Weather ETL Pipeline...")

    # Replace with your real OpenWeatherMap API key
    OPENWEATHER_API_KEY = open_whether_key
    ...
    weather_dfs = [
        ...
        # clean_weather_data(sql_data, "SQL", temp_col="temperature", temp_unit='C', humidity_col="humidity", wind_col="wind_speed", location_col="station"),
